@testset "Bug E — tree pane survives post-launch node additions" begin
    ...
    try
        s = MakieViews._current_session[]
        n0 = length(MakieViews._build_tree_rows(s)[1])
        ...
        fig2 = MakieViews.add_figure!(s; title = "post-launch fig")     # must not throw
        sleep(0.2)
        
        @test length(MakieViews._build_tree_rows(s)[1]) > n0            # rows grew
    finally
        Gtk4.destroy(w)
    end
end
